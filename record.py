...
obd.logger.setLevel(obd.logging.DEBUG)  #//enable console printing of all debug>
...
with open ('DATA_'+str(ctime).replace(' ','_')+'.csv', 'w') as f:
        ...
        try:
                while True:
                        ...
                        apiList.append("{" + f"\"time\": {timecount}, \"speed\": {speed_value}, \"rpm\": {rpm_value}, \"coolant\": {coolant_value}, \"maf\": {maf_value}, \"air_status\": {air_status_value}, \"intake_pressure\": {intake_pressure_value}, \"throttle_position\": {throttle_position_value}" + "}")

                        time.sleep(1.0)
        except KeyboardInterrupt:
                pass

response = requests.post('https://thesis-api.vercel.app/api/post_data', data = {"car_id": 1, "car_data": apiList})
print(response)
